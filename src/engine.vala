...
namespace Kiro {
    public class TerminalEngine : Object {
        ...
        private void setup_terminal() {
            ...
            // Bell urgent functionality - check setting at runtime
            ...
            if (settings.allow_bold) {
                ...
                css_provider.load_from_string("terminal text { font-weight: bold; }");
                Gtk.StyleContext.add_provider_for_display(terminal.get_display(), css_provider, Gtk.STYLE_PROVIDER_PRIORITY_APPLICATION);
            }
        ...
        public void increase_font_size() {
            var font_desc = terminal.get_font().copy();
        ...
        private void update_colors() {
            ...
            if (settings.use_theme_colors) {
                // Use Adw.StyleManager for modern theme detection
                var style_manager = Adw.StyleManager.get_default();
                if (style_manager.dark) {
                    foreground = {0.9f, 0.9f, 0.9f, 1.0f};
                    background = {0.1f, 0.1f, 0.1f, 1.0f};
                } else {
                    foreground = {0.15f, 0.15f, 0.15f, 1.0f};
        ...
        private void on_settings_changed(string key) {
            switch (key) {
                ...
                case "scrollback-lines":
                    ...
                    break;
                case "mouse-autohide":
                    // Motion controller already checks settings.mouse_autohide at runtime
                    break;
                case "allow-bold":
                    var css_provider = new Gtk.CssProvider();
                    if (settings.allow_bold) {
                        ...
                        css_provider.load_from_string("terminal text { font-weight: normal; }");
                    }
                    Gtk.StyleContext.add_provider_for_display(terminal.get_display(), css_provider, Gtk.STYLE_PROVIDER_PRIORITY_APPLICATION);
                    break;
                ...
                case "scroll-on-keystroke":
                    ...
                    break;
                case "urgent-bell":
                    // Bell signal already checks settings.urgent_bell at runtime
                    break;
                case "word-chars":
                    terminal.set_word_char_exceptions(settings.word_chars);
                    break;
                    ...
                    if (n_press == 1) { // Single click
                        ...
                        if (match != null && match.length > 0) {
                            ...
                            if (match.has_prefix("http://") || match.has_prefix("https://") ||
                                ...
                                launcher.launch.begin(terminal.get_root() as Gtk.Window, null, (obj, res) => {
                                    try {
                                        launcher.launch.end(res);
                                    } catch (Error e) {
                                        warning("Failed to open URL '%s': %s", match, e.message);
                                    }
                                });
                                ...
                                if (file.query_exists()) {
                                    ...
                                    launcher.launch.begin(terminal.get_root() as Gtk.Window, null, (obj, res) => {
                                        try {
                                            ...
                                            warning("Failed to open file '%s': %s", match, e.message);
                                        }
                                    });
                                }
                            }
