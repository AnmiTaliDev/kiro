...
namespace Kiro {
    public class MainWindow : Adw.ApplicationWindow {
        ...
        public void show_preferences() {
            if (preferences_window == null) {
                preferences_window = new PreferencesWindow(this, settings);
        ...
        private void apply_window_settings() {
            ...
            if (settings.window_transparency > 0.0) {
                ...
                try {
                    ...
                    warning("Failed to apply window transparency: %s", e.message);
                }
            }
    ...
    public class PreferencesWindow : Adw.PreferencesWindow {
        ...
        public PreferencesWindow(Gtk.Window parent, Settings settings) {
            Object(
        ...
        private void add_appearance_groups(Adw.PreferencesPage page) {
            ...
            var fg_color = new Gtk.ColorButton();
            var fg_rgba = Gdk.RGBA();
            fg_rgba.parse(settings.foreground_color);
            fg_color.rgba = fg_rgba;
            fg_color.color_set.connect(() => {
                settings.foreground_color = fg_color.rgba.to_string();
            });
            ...
            var bg_color = new Gtk.ColorButton();
            var bg_rgba = Gdk.RGBA();
            bg_rgba.parse(settings.background_color);
            bg_color.rgba = bg_rgba;
            bg_color.color_set.connect(() => {
                settings.background_color = bg_color.rgba.to_string();
            });
